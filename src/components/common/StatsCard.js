...
const StatsCard = ({ title, value, icon, trend, style }) => (
 ...
 <Text className={`text-xs font-times font-bold ${trend.startsWith('+') || trend === 'Stable' ? 'text-emerald-500' : 'text-rose-500'}`}>{trend}</Text>
 </View>
 )}
 </View>
 <View>
 <Text className="text-slate-500 text-xs font-times font-bold uppercase tracking-[0.3em] mb-2 ">{title}</Text>
 <Text className="text-white text-3xl font-times font-bold tracking-tighter uppercase">{value}</Text>
 </View>
 </GlassCard>
 </TouchableOpacity>
);
