...
export const generateSmartInsights = (income, expenses, savings, healthScore, gap) => {
 ...
 insights.push({
 type: 'alert',
 text: `Expenses exceed income. You have a deficit of ₹${Math.abs(savings).toLocaleString('en-IN')}. Review your spending categories.`,
 category: 'Savings'
 });
 } else if (savings > 0) {
 insights.push({
 type: 'positive',
 text: `You have ₹${savings.toLocaleString('en-IN')} available for investment. Consider allocating this to an SIP.`,
 category: 'Savings'
 });
 }
 ...
 insights.push({
 type: 'critical',
 text: `Emergency Fund Gap Detected: Shortfall of ₹${gap.toLocaleString('en-IN')} to reach 6-month safety buffer.`,
