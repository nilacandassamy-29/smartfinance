...
export const getCategoryDetails = (category) => {
  ...
  if (cat.includes("food") || cat.includes("foof"))
    return {
      bg: "#fff7ed",
      icon: <UtensilsCrossed size={20} color="#f97316" />,
    };
  if (cat.includes("general"))
    return { bg: "#eff6ff", icon: <Wallet size={20} color="#3b82f6" /> };
  if (cat.includes("personal"))
    return { bg: "#faf5ff", icon: <User size={20} color="#a855f7" /> };
  if (cat.includes("transport"))
    return { bg: "#f0f9ff", icon: <Car size={20} color="#0ea5e9" /> };
  if (cat.includes("bill"))
    return { bg: "#fefce8", icon: <Zap size={20} color="#eab308" /> };
  if (cat.includes("shop"))
    return { bg: "#fdf2f8", icon: <ShoppingBag size={20} color="#ec4899" /> };
...
export default function Dashboard({ navigation }) {
  ...
  const dynamicPortfolio = Math.max(
    0,
    (monthlyIncome || 0) - (monthlyTotal || 0),
  );

  let badgeText = "New";
  let badgeColor = isDarkMode
        ...
        >
          ...
          >
            ...
            {isLoadingData ? (
              ...
              >
                ₹
                {(dynamicPortfolio || 0).toLocaleString("en-IN", {
              ...
              {recentTransactions.map((exp, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View
                      style={{ flex: 1, marginRight: 8, overflow: "hidden" }}
                    >
                      <Text
                        style={{
                          fontFamily: "Poppins-SemiBold",
