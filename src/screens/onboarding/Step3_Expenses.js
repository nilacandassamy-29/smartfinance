...
const Step3_Expenses = () => {
    ...
    const expenseCategories = [
        ...
        { id: 'mobile', label: 'Cellular', icon: <Smartphone size={14} />, color: 'bg-slate-500/10', border: 'border-slate-500/20', textColor: 'text-slate-400' },
        ...
        { id: 'miscellaneous', label: 'Entropy', icon: <MoreHorizontal size={14} />, color: 'bg-slate-500/10', border: 'border-slate-500/20', textColor: 'text-slate-400' },
    ];
    ...
    return (
        <OnboardingLayout currentStep={3}>
                ...
                {expenseCategories.map((cat, idx) => (
                    ...
                    >
                        <View className="flex-row items-center space-x-3 mb-4">
                            ...
                            <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">{cat.label}</Text>
                        </View>
                        <View className="flex-row items-center border-b border-white/5 pb-1">
                            <Text className="text-slate-600 font-black text-xs italic mr-2 uppercase">Amount</Text>
                            <TextInput
                                className="flex-1 font-black text-xl text-white p-0 italic"
                ...
                <BlurView intensity={30} tint="dark" className="p-8 bg-slate-900/40">
                    <View className="flex-row justify-between mb-8">
                        <View>
                            <View className="flex-row items-center space-x-2 mb-2">
                                ...
                                <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em]">Inflow</Text>
                            </View>
                            <Text className="text-3xl font-black text-white italic tracking-tighter">₹{totalIncome.toLocaleString()}</Text>
                        </View>

                        <View className="items-end">
                            <View className="flex-row items-center space-x-2 mb-2">
                                <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em]">Outflow</Text>
                            ...
                            <Text className="text-3xl font-black text-white italic tracking-tighter">₹{totalExpenses.toLocaleString()}</Text>
                        </View>
                    </View>
                    ...
                    <View className={`p-6 rounded-[2rem] border-2 flex-row justify-between items-center ${isDeficit ? 'bg-rose-500/10 border-rose-500/20 shadow-rose-500/10' : 'bg-emerald-500/10 border-emerald-500/20 shadow-emerald-500/10'}`}>
                        <View className="flex-row items-center space-x-5">
                            ...
                            <View>
                                <Text className="text-[10px] font-black uppercase tracking-[0.3em] italic text-slate-500 mb-1">
                                    {isDeficit ? 'Deficit Delta' : 'Net Surplus'}
                                </Text>
                                <Text className={`text-2xl font-black italic tracking-tighter ${isDeficit ? 'text-rose-400' : 'text-emerald-400'}`}>
                        ...
                        <View className={`px-4 py-2 rounded-xl border ${isDeficit ? 'bg-rose-500/20 border-rose-500/30' : 'bg-emerald-500/20 border-emerald-500/30'}`}>
                            <Text className={`text-[9px] font-black uppercase tracking-widest italic ${isDeficit ? 'text-rose-400' : 'text-emerald-400'}`}>{isDeficit ? 'CRITICAL' : 'OPTIMAL'}</Text>
                        </View>
                    </View>
                </BlurView>
            </MotiView>
            ...
            >
                <Text className="text-white font-black text-xs uppercase tracking-[0.3em] italic">Synthesize Intelligence</Text>
