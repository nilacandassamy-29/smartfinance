        ...
        <OnboardingLayout currentStep={2}>
            <View className="mb-10 flex-row items-center justify-between">
                ...
                <BlurView intensity={20} tint="dark" className="px-4 py-2 rounded-xl border border-white/5 overflow-hidden">
                    <Text className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.3em]">
            ...
            <View className="mb-12">
                <Text className="text-4xl font-black text-white tracking-tighter uppercase italic">
            ...
            <AnimatePresence exitBeforeEnter>
                ...
                >
                    ...
                    <View className="space-y-8">
                        ...
                        <View>
                            <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-3 ml-1">Cellular Classification (Gender) *</Text>
                    ...
                    <View className="mt-4">
                        <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4 ml-1">Functional Directive (Occupation)</Text>
                        <View className="flex-row flex-wrap gap-4">
                            {occupations.map((occ) => (
                                ...
                                >
                                    ...
                                    <Text className={`font-black text-[9px] uppercase tracking-[0.2em] italic ${activeMember.occupation === occ.id ? 'text-white' : 'text-slate-500'}`}>{occ.label}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                    </View>
                    ...
                    {activeMember.occupation === 'Student' && (
                        <MotiView from={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                            <View>
                                <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-3 ml-1">Academic Tier</Text>
                                <View className="flex-row space-x-3">
                                    {['School Student', 'College Student'].map(type => (
                                        ...
                                        >
                                            <Text className={`text-[9px] font-black uppercase tracking-widest ${activeMember.educationType === type ? 'text-white' : 'text-slate-500'}`}>{type}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                            </View>
                            ...
                            <View className="p-6 bg-indigo-500/10 rounded-[2rem] border border-indigo-500/20">
                                <Text className="text-[9px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-4">Cumulative Fees (Annual)</Text>
                    ...
                    {(activeMember.occupation === 'Working' || activeMember.occupation === 'Retired') && (
                        <MotiView from={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                            <View>
                                <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-1 ml-1">Economic Sector</Text>
                                <View className="w-full h-14 bg-white/5 rounded-[1.25rem] border border-white/5 justify-center px-2">
                                    <Picker
                            ...
                            <View className="p-6 bg-emerald-500/5 rounded-[2rem] border border-emerald-500/10">
                                <Text className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-400 mb-4">Monthly Inflow (Income)</Text>
                    ...
                    >
                        <Text className={`font-black text-xs uppercase tracking-[0.3em] italic ${isStepValid() ? 'text-white' : 'text-slate-600'}`}>
...
const InputField = ({ label, icon, value, onChangeText, placeholder, keyboardType = 'default', error }) => (
    <View>
        <Text className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-3 ml-1">{label}</Text>
        <View className={`flex-row items-center space-x-4 bg-white/5 h-14 rounded-[1.25rem] px-5 border ${error ? 'border-rose-500' : 'border-white/5'}`}>
            {icon}
            <TextInput
                className="flex-1 font-black text-sm text-white"
        ...
        {error && <Text className="text-[8px] font-black text-rose-500 uppercase tracking-widest mt-1.5 ml-1">Protocol field invalid</Text>}
    </View>
);
